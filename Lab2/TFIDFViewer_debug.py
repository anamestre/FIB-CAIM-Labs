...
def normalize(tw):
    ...
    :return:
    """
    print('Vector antes de la normalización:')
    print_term_weigth_vector(tw)
    print('===========================================')
    ...
    s = sum(list(map(lambda x: x[1]**2, tw)))   # L'ús de funcions d'ordre superior es equivalent a:
                                                #    for (_,w) in tw:
                                                #       s += w*w
    print('Suma de todos los pesos al cuadrado: ', s)

    r = np.sqrt(s) 
    print('Raiz cuadrada de la suma: ', r)
    ...
    print('Vector después de la normalización:')
    print_term_weigth_vector(norm)
    print('===========================================')
    return norm
...
def cosine_similarity(tw1, tw2):
    ...
    while index_tw1 < len(tw1) and index_tw2 < len(tw2):    # Mentres que hi hagi algún element per visitar
        ...
        if t1 == t2:    # Si són el mateix terme, incrementem el sim i avançem els dos índex al mateix temps
            print('Ambos documentos tienen el mismo termino: ', t1,' y sus pesos son: ', w1,
                    ' para el primero documento y ',w2, ' para el segundo')
            sim += w1 * w2
...
if __name__ == '__main__':
    ...
    try:
        ...
        print ('Información del documento: ', file1)
        file1_tw = toTFIDF(client, index, file1_id)
        print ('Información del documento: ', file2)
